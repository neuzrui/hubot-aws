...
#
# Commands:
#   hubot ec2 start --instance_id=[instance_id] - Start the Instance
#
# Notes:
...
module.exports = (robot) ->
  robot.respond /ec2 start(.*)$/i, (msg) ->
    ...
    msg.send "Starting instance_id=#{ins_id}, dry-run=#{dry_run}..."

    aws = require('../../aws.coffee').aws()
    ec2 = new aws.EC2({apiVersion: '2014-10-01'})

    ec2.startInstances { DryRun: dry_run, InstanceIds: [ins_id] }, (err, res) ->
      if err
        msg.send "Error: #{err}"
      else
        messages = []
        for ins in res.StartingInstances
          id     = ins.InstanceId
